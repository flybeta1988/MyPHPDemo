...
class OrgCourseMiddle
{
    public static function getGradeRangeStr (array $org_class) {
        ...
        $suitable_min = isset($org_class['suitable_min']) ? (int)$org_class['suitable_min'] : 0;
        $suitable_max = isset($org_class['suitable_max']) ? (int)$org_class['suitable_max'] : 0;

        //年龄需要转换成理论年级
        if (1 == $suitable_type) {
            $grade_min = self::getGradeIntByAge($suitable_min);
            $grade_max = self::getGradeIntByAge($suitable_max);
        } else {
            $grade_min = $suitable_min;
            $grade_max = $suitable_max;
        }
